...
(function($) {
    var methods = {
        pageInit: function(options) {
             ...
             * @type {Obeject}
             */
            var opts = $.extend({}, $.fn.sPager.defaults, options);
        ...
        dataRender: function(_v, _data) {
            ...
            cells += '<caption>ANALYSIS</caption>'+'<thead>' + '<tr>' + '<th>Username</th>'+ '<th>Age</th>' + '<th>Phonenumber</th>' + ' <th>locked</th>' + '</tr>' + '</thead><tbody>';
            for (var i = j * 8; i < temp; i++) {
                cells += '<tr><td>' + _data[i].realname  + '</td>';
                cells += '<td>' + _data[i].mid + '</td>';
                cells += '<td>' + _data[i].mobile + '</td>';
                cells += '<td>' + _data[i].locked + '</td>';
                cells += '</tr>';
            }
            cells += '</tbody>';
        ...
        ajaxData: function(_url, _current) {
             ...
             */
            var _total = $.fn.sPager.defaults.totalData;
            return (function() {
    ...
    $.extend({
        pageError: function(type) {
            ...
            switch (type) {
                case 1:
                    console.log('method' + method + 'dose not exist on jQuery.sPager');
                    break;
                case 2:
                    console.log('no ajax');
                    break;
