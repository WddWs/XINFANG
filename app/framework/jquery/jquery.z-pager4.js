...
(function($) {
    var methods = {
        pageInit: function(options) {
             ...
             * @type {Obeject}
             */
            var opts = $.extend({}, $.fn.qPager.defaults, options);
        ...
        dataRender: function(_v, _data) {
            ...
            cells += '<caption>NEWS</caption>'+'<thead>' + '<tr>' + '<th>State</th>'+ '<th>Locked</th>' + '<th>Id</th>' + ' <th>Dstate</th>' + '</tr>' + '</thead><tbody>';
            for (var i = j * 8; i < temp; i++) {
                cells += '<tr><td>' + _data[i].state  + '</td>';
                cells += '<td>' + _data[i].locked + '</td>';
                cells += '<td>' + _data[i].id + '</td>';
        ...
        ajaxData: function(_url, _current) {
             ...
             */
            var _total = $.fn.qPager.defaults.totalData;
            return (function() {
    ...
    $.extend({
        pageError: function(type) {
            ...
            switch (type) {
                case 1:
                    console.log('method' + method + 'dose not exist on jQuery.qPager');
                    break;
                case 2:
                    console.log('no ajax');
                    break;
