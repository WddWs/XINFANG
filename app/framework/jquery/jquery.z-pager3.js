...
(function($) {
    var methods = {
        pageInit: function(options) {
             ...
             * @type {Obeject}
             */
            var opts = $.extend({}, $.fn.aPager.defaults, options);
        ...
        dataRender: function(_v, _data) {
            ...
            var cells = '';
            var temp = 0;
            maxpage = parseInt(_data.length / 8);
            maxpagenum = _data.length % 8;
            ...
            cells += '<caption>DATA</caption>'+'<thead>' + '<tr>' + '<th>Username</th>'+ '<th>compName</th>' + '<th>startTime</th>' + ' <th>dstate</th>' + '</tr>' + '</thead><tbody>';
            for (var i = j * 8; i < temp; i++) {
                cells += '<tr><td>' + _data[i].realname  + '</td>';
                cells += '<td>' + _data[i].title + '</td>';
                cells += '<td>' + _data[i].starttime + '</td>';
                cells += '<td>' + _data[i].dstate + '</td>';
                cells += '</tr>';
            }
            cells += '</tbody>';
            if (opts.htmlBox === '' || (typeof(opts.htmlBox) !== 'Obeject')) {
                var abx = _v.prev();
                if (!abx.hasClass('table table-hover')) {
                    var d = '<table class="table table-hover"></table>';
                    _v.before(d);
                }
                _v.prev().html(cells);
            } else {
            ...
            $.each(_a, function(index, item) {
                ...
                _this.on("click", function() {
                    ...
                    if (_this.attr("disabled")) {
                        ...
                        if (_this.attr("class") == "pg-next") {
                            j = j + 1;
                        }
                        if (_this.attr("class") == "pg-prev") {
                            j = j - 1;
                        }
                        if (_this.attr("class") == "pg-first") {
                            j = 0;
                        }
                        if (_this.attr("class") == "pg-last") {
                            j = maxpage;
                        }
                    }
        ...
        pageBtn: function(_id) {
             ...
             * @type {String}
             */
            return '<a class="" page-id="' + _id + '">' + _id + '</a>';
        },
        ...
        ajaxData: function(_url, _current) {
             ...
             */
            var _total = $.fn.aPager.defaults.totalData;
            return (function() {
    ...
    $.extend({
        pageError: function(type) {
            ...
            switch (type) {
                case 1:
                    console.log('method' + method + 'dose not exist on jQuery.aPager');
                    break;
                case 2:
                    console.log('no ajax');
                    break;
